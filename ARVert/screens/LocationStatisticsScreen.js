...
import { getVisitsList, getVisitsListLocation } from "../actions/visitsActions";
...
function LocationStatisticsScreen (props) {
  const sexData = [
    {
      seriesName: 'Male',
      data: [
  ...
  useEffect(() => {
    if (props.visitsListLocation && !chart.loaded) {
      let totalLocal = props.visitsListLocation.length;
      let sexLocal = sexData;
      let monthlyLocal = monthlyData;
      let citiesLocal = citiesData;

      for (let i = 0; i < props.visitsListLocation.length; i++) {
        const month = props.visitsListLocation[i].data.VisitedAt;
        const age = parseInt(props.visitsListLocation[i].data.User.Age, 10);
        const gender = props.visitsListLocation[i].data.User.Gender;
        const origin = props.visitsListLocation[i].data.User.Origin;
        switch (month) {
          case 'January':
            monthlyLocal[0].y++;
            break;
    ...
    } else if (props.locationId && props.visitsListLocation === undefined) {
      props.getVisitsListLocation({ location: props.locationId });
    }
  });
  ...
  return (
    <View style={[styles.inlineHeader]}>
      <View>
        ...
        <Text style={[TextStyles.caption, styles.caption]}>{props.location.Name} – <Text>Views: {total}</Text></Text>
      </View>
    </View>
  )
}

export const LocationStatisticsScreenWithRedux = connect(state => ({
  ...state.visitsReducer,
  ...state.locationReducer
}), {
  getVisitsList,
  getVisitsListLocation
})(LocationStatisticsScreen);

LocationStatisticsScreenWithRedux.navigationOptions = {
  header: null,
  headerMode: 'none',
};
